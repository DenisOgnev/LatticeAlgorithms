...
namespace Utils
{
    ...
    Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> add_column(const Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> &H, const Eigen::Vector<int64_t, Eigen::Dynamic> &b_column)
    {
        ...
        Eigen::Vector<int64_t, Eigen::Dynamic> H_first_col = H.col(0);

        int64_t a = H_first_col(0);
        Eigen::Vector<int64_t, Eigen::Dynamic> h = H_first_col.tail(H_first_col.rows() - 1);
        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> H_stroke = H.block(1, 1, H.rows() - 1, H.cols() - 1);
        int64_t b = b_column(0);
        Eigen::Vector<int64_t, Eigen::Dynamic> b_stroke = b_column.tail(b_column.rows() - 1);

        std::tuple<int64_t, int64_t, int64_t> gcd_result = gcd_extended(a, b);
        int64_t g, x, y;
        std::tie(g, x, y) = gcd_result;

        Eigen::Matrix2d U;
        U << x, -b / g, y, a / g;
        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> temp_matrix(H.rows(), 2);
        temp_matrix.col(0) = H_first_col;
        temp_matrix.col(1) = b_column;
        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> temp_result = (temp_matrix.cast<double>() * U).cast<int64_t>();

        Eigen::Vector<int64_t, Eigen::Dynamic> h_stroke = temp_result.col(0).tail(temp_result.rows() - 1);
        Eigen::Vector<int64_t, Eigen::Dynamic> b_double_stroke = temp_result.col(1).tail(temp_result.rows() - 1);

        b_double_stroke = reduce(b_double_stroke, H_stroke);

        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> H_double_stroke = add_column(H_stroke, b_double_stroke);

        h_stroke = reduce(h_stroke, H_double_stroke);

        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> result(H.rows(), H.cols());
    ...
    {
        Eigen::Vector<int64_t, Eigen::Dynamic> result = vector;
        for (int i = 0; i < result.rows(); i++)
        {
            Eigen::Vector<int64_t, Eigen::Dynamic> matrix_column = matrix.col(i);
            double vec_elem = static_cast<double>(result(i));
            double matrix_elem = static_cast<double>(matrix(i, i));
            int64_t x;
            if (vec_elem < 0)
            {
                x = static_cast<int64_t>(std::ceil(std::abs(vec_elem) / matrix_elem));
                result += matrix_column * x;
                vec_elem = static_cast<double>(result(i));
            }
            if (vec_elem >= matrix_elem)
            {
                x = static_cast<int64_t>(std::floor(vec_elem / matrix_elem));
                result -= matrix_column * x;
            }
    ...
    // @param matrix input matrix
    // @return std::tuple<Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic>, Eigen::MatrixXd, std::vector<int>>
    std::tuple<Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic>, Eigen::MatrixXd, std::vector<int>> get_linearly_independent_columns_by_gram_schmidt(const Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> &matrix)
    {
        ...
        int counter = 0;
        for (const Eigen::Vector<int64_t, Eigen::Dynamic> &vec : matrix.colwise())
        {
        ...
        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> result(matrix.rows(), indexes.size());
    ...
    // @param matrix input matrix
    // @return std::tuple<Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic>, std::vector<int>>
    std::tuple<Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic>, std::vector<int>> get_linearly_independent_rows_by_gram_schmidt(const Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> &matrix)
    {
        ...
        int counter = 0;
        for (const Eigen::Vector<int64_t, Eigen::Dynamic> &vec : matrix.rowwise())
        {
            Eigen::VectorXd projections = Eigen::VectorXd::Zero(vec.size());
        ...
        Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> result(indexes.size(), matrix.cols());
        //#pragma omp parallel for
        for (int i = 0; i < indexes.size(); i++)
        {
    ...
    std::string matrix_to_string(const Eigen::Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic> &matrix)
    {
        ...
        for (const auto &vec : matrix.colwise())
        {
            ...
            {
                result += std::to_string(static_cast<int64_t>(elem)) + ", ";
            }
            result.pop_back();
            if (!result.empty())
            {
